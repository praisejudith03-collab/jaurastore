...
**`ADMIN_MASTER_PASSWORD` — the primary admin credential.** A dedicated environment variable that signs in to any configured admin account with **no database involved** (the repository's auth is environment-backed: `main` moved to a single permanent env credential and removed the legacy DB-password/OTP/email flows on 2026-09-09; this branch follows that direction). `ADMIN_MASTER_PASSWORD` is the **primary** master password; `ADMIN_BOOTSTRAP_PASSWORD` is the **secondary/permanent fallback** — both read live from the environment on every attempt, so updating either in Render → Environment works on the very next login, no database update, no restart. Guardrails: constant-time comparison, only opens `ADMIN_EMAILS` accounts (no enumeration vector), identical wrong-password 401s, rate-limited, audit-logged as `admin.login · master password`. Documented in `.env.example`, `render.yaml` (both services), `ENVIRONMENT_VARIABLES.md`. Pinned by `tests/test_admin_master_password.py`.
